...
// 查询判题列表
func ListJudgeHandler(svcCtx *svc.ServiceContext) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		var req types.ListJudgeReq
		if err := httpx.Parse(ctx, &req); err != nil {
			httpx.ErrorCtx(ctx, err)
			return
		}

		l := judge.NewListJudgeLogic(ctx, svcCtx)
		resp, err := l.ListJudge(&req)
		if err != nil {
			httpx.ErrorCtx(ctx, err)
		} else {
